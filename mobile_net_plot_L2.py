...
non_destination_node_list = [x for x in all_nodes_list if x not in destination_node_list]
# draw non-origin nodes
#nx.draw_networkx_nodes(G=graph, pos=pos, nodelist=non_origin_node_list,  # nodelist=graph.nodes(),
#                       node_color='#960340', alpha=0.8, node_size=22)
nx.draw_networkx_nodes(G=graph, pos=pos, nodelist=non_destination_node_list,  # nodelist=graph.nodes(),
                       node_color='#026aa6', alpha=0.8, node_size=22)
# draw origin nodes
#nx.draw_networkx_nodes(G=graph, pos=pos, nodelist=origin_node_list,  # nodelist=graph.nodes(),
#                       node_color='#026aa6', alpha=0.8, node_size=24)

nx.draw_networkx_nodes(G=graph, pos=pos, nodelist=destination_node_list,  # nodelist=graph.nodes(),
                       node_color='#960340', alpha=0.8, node_size=22)
...
legend_elements = [Line2D([0], [0], marker='o', color='w', label='Origin',
                          markerfacecolor='#026aa6', markersize=10),
                   Line2D([0], [0], marker='o', color='w', label='Destination',
                          markerfacecolor='#960340', markersize=10)]
